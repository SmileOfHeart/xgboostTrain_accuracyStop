...
class TrainLine(gym.Env):
    ...
    def __init__(self):
        self.action_space = ['-0.3', '-0.2', '-0.1', '0', '0.1', '0.2', '0.3']
        self.S = 1275
    ...
    def step(self, action):
        ...
        action = (action - 3) / 10  # 转化为[-0.3, 0.3]
        u = round((self.u + action*self.t), 1)
        u = max(-0.8, u)
        u = min(0.8, u)
        self.u =u
        ...
        position1 += pos_cha
        velocity1 += self.t * u
        if (velocity1 > self._get_max_speed(position1)):
            self.u = -0.8
            velocity1 = self._get_max_speed(position1)
...
#            reward += -0.3*(abs(self.step1 * self.t - self.T)) ** 2  #？？？
        ...
        if u >= 0:
#           if v_dif < 0:
           reward0 += -(u + self.resistance) * pos_cha * 1.0    #做功
           self.energy += (u + self.resistance) * pos_cha     #能耗
           reward2 += pos_cha * 0.2
           #位移
        else:
           reward0 = 0           
#        reward -= abs(v_dif)**2 * 0.1
#        Tbar = self.step1 *self.t + self.get_refer_time(position1)  
        Tbar = self.step1 *self.t
        d1dv = min(1/(velocity1 + 0.1),300)
        reward1 += -1*(2*d1dv * Tbar-2*self.T*d1dv + self.T**2/1274.5) * pos_cha * 5.0  #时间        
        reward += reward0 +reward1 + reward2
        self.state1 = (position1, velocity1)
#        self.state = (position, velocity, v_dif)
        self.state = (position,velocity,self.step1 * self.t,self.u)
        self.step1 += 1
        return np.array(self.state),self.energy,reward, done, {}

    def reset(self):  #初始位置
        self.Tr = self.step1 * self.t
        self.energy = 0
        self.step1 = 0
        self.u = 0.8
        self.state = np.array([1.6, 1.6, 0, 0.3])   #创建一个数组
