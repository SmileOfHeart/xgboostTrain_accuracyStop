...
def plotSpeedLimitRoadGrad(style):
    #style:relative :相对坐标
    #      abstract: 绝对坐标
    Mat=tool.ReadCSVMat('speedLimit_ciqu')
    startPoint=Mat[:,0]
...
def ReadBrakeSpeedLimitCurve():
    ...
    with open('speedLimit_ciqu.csv',mode='r',encoding='UTF-8-sig') as file_obj:
...
def ReadMinTimeCurve():
    #读取存储在文本中的紧急制动曲线信息,返回值 BLKDic
    with open('minTimeCurve_ciqu.csv',mode='r',encoding='UTF-8-sig') as file_obj:
        contents=file_obj.readlines()
